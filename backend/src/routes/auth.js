...
router.post('/register', async (req, res) => {
  ...
  if (password.length < 6) {
    return res.status(400).json({
      message: 'A password deve ter mínimo 6 caracteres'
    });
  }
  ...
  try {
    ...
    await db.query(
      'INSERT INTO users (nome, email, password, otp_code, otp_expires_at) VALUES (?,?,?,?,?)',
    ...
    res.json({
      message: 'Registo efectuado. Verifique o email.'
    });
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    await db.query(
      'UPDATE users SET verified = 1, otp_code = NULL, otp_expires_at = NULL WHERE id = ?',
      [user.id]
    );
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    console.log('Password correta?', passwordOk);
