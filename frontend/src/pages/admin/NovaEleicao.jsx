...
export default function NovaEleicao() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <h2 className="text-2xl font-bold text-white mb-8">Nova Eleição</h2>

      <div className="flex flex-col gap-6">
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 flex flex-col gap-4">
          <h3 className="font-semibold text-white">Informação Geral</h3>
          <Field label="Título">
          ...
          <Field label="Descrição (opcional)">
            <textarea value={descricao} onChange={e => setDescricao(e.target.value)}
              ...
              className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3.5 py-2.5 text-sm outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-600 resize-none h-24" />
          </Field>
        ...
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-white">Candidatos</h3>
            <button onClick={addCandidato} className="text-sm text-indigo-400 hover:text-indigo-300">+ Adicionar</button>
          </div>
          {candidatos.map((c, i) => (
            <div key={i} className="p-4 bg-slate-900 rounded-xl border border-slate-700">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-slate-400 font-medium uppercase tracking-wider">Candidato {i + 1}</span>
                {candidatos.length > 2 && (
                  <button onClick={() => removeCandidato(i)} className="text-xs text-red-400 hover:text-red-300">Remover</button>
                )}
              </div>
